  ...
  func classify(image: UIImage, completion: @escaping ((Result<String>) -> ())) {
    DispatchQueue.global(qos: .background).async {
      ...
      // Postprocessing: Find the label with highest confidence and return as human readable text.
        ...
        if let path = Bundle.main.path(forResource: "quickdraw_labels", ofType: "json") {
      ...
      let maxConfidence = results.max() ?? -1
        let top3 = sortedResults.suffix(3).reversed()
      let maxIndex = results.firstIndex(of: maxConfidence) ?? -1
        print(results.count)
        print(maxIndex)
        let resultName = nodes["\(maxIndex)"] ?? ""
      var humanReadableResult = "Predicted: \(resultName)\nConfidence: \(maxConfidence) \n"
        
        for confidence in top3 {
            let doubleStr = String(format: "%.2f", confidence)
            let maxIndex = results.firstIndex(of: confidence) ?? -1
           
            
            let resultName = nodes["\(maxIndex)"] ?? ""
            humanReadableResult += "Predicted: \(resultName) Confidence: \(doubleStr) \n"
...
private enum Constant {
  ...
  static let modelFilename = "quickdraw-acc94"
